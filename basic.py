...
def login():
    if request.method == 'POST':
        ...
        check_user_exist = "SELECT COUNT(1) FROM dbpm.users_db WHERE username = %s AND password = %s"
        check_root = "SELECT account FROM dbpm.users_db WHERE username = %s AND password = %s"
        val = (user, password)
...
def register():
    if request.method == 'POST':
        ...
        sqlq = "SELECT COUNT(*) FROM dbpm.users_db WHERE username = %s"
        val = (user_reg,)
        ...
        if cursor.fetchone()[0]:
            ...
            execute = "INSERT INTO dbpm.users_db (username, password, email, account) VALUES(%s, %s, %s, %s)"
...
def add_connect():
    if request.method == 'POST':
        ...
        insert_connect = "INSERT INTO dbpm.connects_db(host, username, password, database_name, user_name_table) VALUES (%s, %s, %s, %s, %s)"
        check_db = "SELECT COUNT(1) FROM dbpm.connects_db WHERE database_name = %s"
        value = (data_connect,)
        cursor.execute(check_db, value)
        if cursor.fetchone()[0]:
            flash('You already have db with this name', 'info')
            render_template('add_connect.jinja2')
        else:
            if "admin" in session:
